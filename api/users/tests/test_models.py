...
def test_custom_user_creation(django_user_model):
	...
	user = django_user_model.objects.create_user(email='testuser@example.com', password='securepassword', tenant=tenant)
...
def test_create_user(django_user_model):
	tenant = Tenant.objects.create(name='Test Tenant')
	user = django_user_model.objects.create_user(email='user@example.com', password='securepassword', tenant=tenant)
...
def test_create_user_without_email_raises_error(django_user_model):
	tenant = Tenant.objects.create(name='Test Tenant')
	with pytest.raises(ValueError, match='The Email field must be set'):
		django_user_model.objects.create_user(email=None, password='password', tenant=tenant)
...
def test_create_superuser_without_is_superuser_raises_error():
	...
	with pytest.raises(ValueError, match='Superuser must have is_superuser=True.'):
		manager.create_superuser(
			email='admin@example.com', password='securepassword', tenant=tenant, is_superuser=False
		)
